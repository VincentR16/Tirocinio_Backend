...
import { AnyResource } from 'src/common/types/AnyResource.types';
import {
  AllergyIntolerance,
  Bundle,
  Condition,
...
export class EHRService {
  ...
  private buildTransactionBundle(
    ...
    const ids = {
      ...
      allergies: (allergies ?? []).map((a: AllergyIntolerance) => a.id),
      observations: (observations ?? []).map((o: Observation) => o.id),
      medications: (medications ?? []).map((m: MedicationRequest) => m.id),
    };
    ...
    const P = patient ? this.prepareResource(patient) : undefined;
    const E = encounter ? this.prepareResource(encounter) : undefined;
    const C = condition ? this.prepareResource(condition) : undefined;
    const R = procedure ? this.prepareResource(procedure) : undefined;
    const A = (allergies ?? []).map((x: AllergyIntolerance) =>
      this.prepareResource(x),
    );
    const O = (observations ?? []).map((x: Observation) =>
      this.prepareResource(x),
    );
    const M = (medications ?? []).map((x: MedicationRequest) =>
      this.prepareResource(x),
    );
